...
**CRITICAL: Make sure to inspect the package.json for running commands, default to running bun over npm, and don't make up commands to run**

**CRITICAL: You always never ask to begin coding without verifying with me the plan. You will never create issues and start work immediately; you will always ask if I want to make any adjustments. Only after I tell you that you can start work do you write code.**

**CRITICAL: You must ALWAYS give me your honest opinion. You are free to disagree with me. You will not automatically agree with everything I say or ask for. You will engage in a spirited debate, and defend your opinions until a clear decision is reached.**

**CRITICAL: You must use the context7 mcp to look up the most recent documentation for the technology we use, ie React, Cloudflare Workers / Durable Objects / D1, Bun, etc**

## System Structure

**Base Directory:** `./ProjectMgmt/darren`

**Issue States (Directories):**

- `./ProjectMgmt/darren/open/` - New and unstarted issues
- `./ProjectMgmt/darren/wip/` - Work in progress issues
- `./ProjectMgmt/darren/closed/` - Completed issues
...
- `src/cron/stats-updater.js` - Hourly stats update job
  **Commands Run**: `tsc --noEmit && bun run lint`
  **Result**: Partial - Performance needs optimization
...
- **Implementation Log**: For actual code changes, technical work performed
- **Critical distinction**: If you wrote/modified code or refactored logic, it goes in Implementation Log
...
- **When to log**: Any time the LLM writes/modifies code, refactors logic, debugs, or performs technical work
...
- **Always log implementation work**: Every code change, commands run, or debug session must be logged
...
- [ ] [[ISSUE-XXX-title-c]] - Testing compliation and linting
...
**Commands Run**: \`bun run lint\`
**Result**: Success - All comands passing
**Next**: Implement password reset functionality
EOF

git add ./ProjectMgmt/wip/ISSUE-XXX-*.md
git commit -m "🔧 [ISSUE-XXX] implementation: added auth endpoints"
```
...
```
1. Perform development work (coding, testing compilation and linting, debugging)
...
3. git add ./ProjectMgmt/wip/ISSUE-XXX-*.md
4. git commit -m "📋 [ISSUE-XXX-a] subtask wip: starting logic refactor"
...
7. Verify everything compiles with no typescript errors
